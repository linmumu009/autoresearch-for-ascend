...
| MindSpeed-LLM | Yes, autoresearch runner completed train -> convert -> HF eval -> TSV record. | Deepscaler smoke steady steps around 0.18-0.25 s after warmup; about 10.3 GB allocated HBM. | Best observed raw HF val_loss `12.152827` at `LR=1.2e-4`, 6 steps; base Qwen3 raw HF val_loss `14.977717`. |
...
Current observed MindSpeed best: `lr_1p2em4_6step.env`. It improved the runner
baseline raw HF validation loss by `2.810139` and the base Qwen3-0.6B raw HF
validation loss by `2.824890` under the same fixed evaluation script.
