...
| 版本 | 日期 | 摘要 |
| --- | --- | --- |
| v0.4.7 | 2026-07-04 | 训练并保存 `1.0e-4` 100-step adapter，然后做 held-out scoring 和生成检查；margin 继续提升，但 32 条 greedy 生成仍与 base 完全相同。 |
...
| v0.4.2 | 2026-07-04 | 保存 `2.0e-4` LoRA adapter checkpoint，并做 base-vs-adapter 生成检查；DPO 指标改善明显，但 16 条短 prompt 生成文本完全相同。 |
| v0.4.1 | 2026-07-04 | 用 100-step holdout 确认 ms-swift DPO 的 `2.0e-4` 学习率：eval_loss `2e-8`，eval margin `19.84`。 |
...
| v0.3.11 | 2026-07-03 | 用 6-step 探测 `1.0e-4` 边界；新的最佳 raw HF val_loss 是 `12.381389`。 |
...
| ms-swift DPO LoRA/FSDP2 | 能，已经跑通 Qwen3.6-27B DPO + LoRA + FSDP2。 | 8 NPU；50-step adapter 保存实验约 3m20s，100-step 约 6m14s，单卡 HBM 约 51.2 GiB。 | LR 推荐现在要按目标区分：`2.0e-4` 最大化 DPO margin；`1.0e-4` 是更均衡的 held-out 选择。`1.0e-4` 100-step 会继续提高 margin，但 mean chosen logprob 转为轻微下降；32 条 held-out greedy 生成仍不变。 |
...
当前观测到的 MindSpeed 最佳：`lr_2em4_6step.env`。在同一个固定 HF 验证脚本下，
它相比 runner baseline 的 raw HF validation loss 改善 `3.284013`，相比 base
Qwen3-0.6B 改善 `3.298764`。
