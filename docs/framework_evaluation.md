...
| MindSpeed-LLM | Runs | Deepscaler smoke steady steps around 0.18-0.25 s after warmup; about 10.3 GB allocated HBM. | Best observed converted checkpoint raw HF val_loss `12.037202` vs base `14.977717`. | Best fit for Qwen3-0.6B text SFT among the Ascend frameworks inspected so far. |
...
budget. The 6-step gain continued through `1.3e-4`, with another material
raw-HF gain. MindSpeed-local valid/train loss is still rising, so the
...
choice is to probe `1.4e-4` on 6 steps to bracket the short-budget over-shoot
boundary, then confirm the best region under a longer budget.
